...
def create_uni3d(pc_model, pretrained_pc, drop_path_rate=0):  
    # create transformer blocks for point cloud via timm
    point_transformer = timm.create_model(pc_model)

    # create whole point cloud encoder
    point_encoder = PointcloudEncoder(point_transformer)
    # uni3d model
    model = Uni3D(point_encoder=point_encoder,)
    if pretrained_pc is not None:
        model.load_state_dict(torch.load(pretrained_pc)['module'])
    return model
