...
from network.triplane_encoder import TriplaneEncode
...
class CrossAttention(nn.Module):
    def __init__(self, feature_dim, sketch_dim: int, kernel_size: float = 1.0,
                 ...
                 image_size: int = 8, world_dims: int = 3, drop_out: float = 0.1, triplane: bool = False, triplane_encoder_depth = 4):
        super().__init__()
        self.num_heads = num_heads
        ...
        if vit_global and vit_local:
            ...
            condition_length = 1
        # if self.triplane:
        #     self.triplane_encoder = TriplaneEncode(image_size=self.image_size, depth=triplane_encoder_depth)
        #     condition_length = image_size**3
    ...
    def forward(self, x, sketch_feature, projection_matrix=None, kernel_size=None):
        ...
        # sketch_feature: B, 256, 1280
        # x: B, 256(feature_dim), 8, 8, 8 -> q: B, 512, 256(feature_dim)
        q = self.q(x).reshape(x.shape[0], self.feature_dim, -1).transpose(1,
                                                                          2) + self.voxel_pe.to(x.device).unsqueeze(0)
        
        # if self.triplane:
        #     plane_feat = self.triplane_encoder(sketch_feature)
        #     voxel_points = torch.from_numpy(get_voxel_coordinates(
        #             resolution=x.shape[-1], size=1, homogeneous=False)).to(projection_matrix.device).unsqueeze(0).repeat(x.shape[0], 1, 1)
        #     xyz = xyz_transform(voxel_points)
            
        #     point_code = F.grid_sample(
        #         plane_feat.reshape(x.shape[0] * 3, -1, x.shape[-1], x.shape[-2]),
        #         xyz.float(),
        #         mode='bilinear', padding_mode='border', align_corners=False
        #     ).reshape(x.shape[0], -1, x.shape[-1]*x.shape[-2]*x.shape[-3]).transpose(1, 2)
            
        #     k = self.k(point_code) + self.condition_pe.to(x.device).unsqueeze(0)
        #     v = self.v(point_code) + self.condition_pe.to(x.device).unsqueeze(0)
        ...
        if self.vit_local and not self.vit_global:
            if projection_matrix is not None:
                ...
                # voxel_points: (512, 4, 1)
                # projection_matrix: (B, 1, 3, 4)
                # pc_in_camera: (B, 512, 3, 1)
...
class ResnetBlock(nn.Module):
    def __init__(self, world_dims: int, dim_in: int, dim_out: int, emb_dim: int, dropout: float = 0.1,
        ...
        if use_text_condition is not None:
    ...
    def forward(self, x, time_emb, text_condition=None):
        ...
        if text_condition is not None:
            h = h * self.pcd_mlp(text_condition)[(...,) +
